...
namespace ns3
{
    ...
    FANETAnimationHelper::FANETAnimationHelper(std::string name)
        : m_filename(name)
    {
        interface = new AnimationInterface(name);
    }
    ...
    TypeId FANETAnimationHelper::GetTypeId()
    {
        static TypeId tid =
            TypeId("ns3::FANETAnimationHelper")
                .SetParent<ns3::Object>()
                .AddConstructor<FANETAnimationHelper>();
    ...
    void FANETAnimationHelper::AssignClusterAnim(uint32_t clusterIndex, NodeContainer cluster) {
        ...
        for (uint32_t i = 0; i < cluster.GetN(); i++) {
            ...
            interface->UpdateNodeDescription(cluster.Get(i), "C" + std::to_string(clusterIndex) + "Node-" + std::to_string(i) + "_" + std::to_string(cluster.Get(i)->GetId()));
            interface->UpdateNodeColor(cluster.Get(i), r, g, b);
        }
    ...
    void FANETAnimationHelper::AssignGDTAnim(NodeContainer GDTNode){
        interface->UpdateNodeColor(GDTNode.Get(0), 255, 0, 0);
    }
    ...
    void FANETAnimationHelper::AssignCHAnim(std::vector<Ptr<Node>> CHNodes)
    {
        ...
        {
            interface->UpdateNodeColor(CHNodes[i], 236, 3, 252);
    ...
    void FANETAnimationHelper::UpdateCHAnim(std::vector<Ptr<Node>> CHNodes)
    {
        for (size_t i = 0; i < CHNodes.size(); i++)
        {
            ...
            if (curCHNodes[i]->GetId() != CHNodes[i]->GetId())
            {
                ...
                interface->UpdateNodeColor(curCHNodes[i], clustersColor[i][0], clustersColor[i][1], clustersColor[i][2]);
    ...
    void FANETAnimationHelper::AnimateFANET(Ptr<FANETTopologyHelper> fanet) {
        ...
        interface->EnablePacketMetadata(true);
    }
}